...
function displayCharacter(character) {
  ...
  link.setAttribute(
    "href",
    `character-sheet.html?character=${character.info.uuid}`
  );
  ...
  if (fileList) {
    ...
    for (const file of fileList) {
      ...
      blob.text().then((value) => {
        ...
        if (!currentList) {
          //If local storage object does not exist initialize it
          currentList = [];
          currentList.push(currentObj);
          localStorage.setItem("characterList", JSON.stringify(currentList));
        } else {
          //Check for matching uuid and update that character instead
        ...
        fileCounter--;
        if (fileCounter == 0) loadCharacters();
      });
    }
  }
}
...
selection.addEventListener("click", (elem) => {
  ...
  if (selection == "delete") {
    if (
      confirm(
        `Are you sure you want to delete ${
          elem.target.parentElement.parentElement
            .getElementsByClassName("character-info")
            .item("0")
      ...
      let characterList = JSON.parse(localStorage.getItem("characterList"));
      const deleteId =
        elem.target.parentElement.parentElement.getAttribute("uuid");
    ...
    const downloadId =
      elem.target.parentElement.parentElement.getAttribute("uuid");
    const name = elem.target.parentElement.parentElement
      .getElementsByClassName("character-info")
      .item("0")
      .getElementsByClassName("name")
      .item("0").innerText;
    for (const i in characterList) {
      if (characterList[i].info.uuid == downloadId) {
        downloadCharacter(
          `${name}.character`,
          JSON.stringify(characterList[i])
        );
        break;
      }
    }
